...
def checkForMACMatch(MAC, Major, Minor):

    beaconID = -1
    for beacon in conf.BeaconList:
        #print("beacon from BeaconList=", beacon)
        beaconMAC = beacon[4].replace(":", "")
        beaconMAC = beaconMAC.replace(" ", "")
        #print("MAC/beacon[4]=%s,%s"% (MAC, beaconMAC))
        if (MAC == beaconMAC):
              beaconID = beacon[0]
              break
...
def processiBeaconList(incomingBeaconList, RSSIArray, TimeStampArray, rollingRSSIArray):
    ...
    for beaconEntry in incomingBeaconList:
        ...
        beaconnumber = checkForMACMatch(MAC, Major, Minor)
...
def calculateDistanceWithRSSI(rssi,beaconnumber): 
        ...
        ratio_linear = pow(10, old_div(ratio_db, 10))
...
def getXYFrom3Beacons(beaconnumbera, beaconnumberb, beaconnumberc, rollingRSSIArray):
	...
	try:
		y = old_div(((T * (xb - xc)) - (S * (xb - xa))), (((ya - yb) * (xb - xc)) - ((yc - yb) * (xb - xa))))
		x = old_div(((y * (ya - yb)) - T), (xb - xa))
...
def AlternativeGetXYFrom3Beacons(beaconnumbera, beaconnumberb, beaconnumberc, rollingRSSIArray):
	...
	x = old_div(( ( (pow(dA,2)-pow(dB,2)) + (pow(cx,2)-pow(ax,2)) + (pow(by,2)-pow(ay,2)) ) * (2*cy-2*by) - ( (pow(dB,2)-pow(dC,2)) + (pow(cx,2)-pow(cx,2)) + (pow(cy,2)-pow(by,2)) ) *(2*by-2*ay) ), ( (2*bx-2*cx)*(2*by-2*ay)-(2*ax-2*bx)*(2*cy-2*by) ));

	y = old_div(( (pow(dA,2)-pow(dB,2)) + (pow(cx,2)-pow(ax,2)) + (pow(by,2)-pow(ay,2)) + x*(2*ax-2*bx)), (2*by-2*ay));
